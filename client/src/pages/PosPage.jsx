...
export default function PosPage() {
  ...
  const categoriesQuery = useQuery({
    ...
    queryFn: () =>
      api
        .get("/categories", { params: { active: true } })
  ...
  const flavorsQuery = useQuery({
    ...
    queryFn: () =>
      api
        .get("/flavors", { params: { status: "AVAILABLE", size: 100 } })
        .then((response) => response.data.data),
  });
  const toppingsQuery = useQuery({
    ...
    queryFn: () =>
      api
        .get("/toppings", { params: { status: "AVAILABLE", size: 100 } })
        .then((response) => response.data.data),
  });
  useEffect(() => {
  ...
  useEffect(() => {
    if (!productsQuery.data || !flavorsQuery.data || !toppingsQuery.data)
      return;
    const flavorPrices = new Map(
      flavorsQuery.data.map((item) => [item.id, item.extraPrice]),
    );
    const toppingPrices = new Map(
      toppingsQuery.data.map((item) => [item.id, item.price]),
    );
    setCart((current) => {
      let changed = false;
      const next = current.map((line) => {
        ...
        const selectedFlavorPrices = line.flavorIds.map((id) =>
          flavorPrices.get(id),
        );
        const selectedToppingPrices = line.toppingIds.map((id) =>
          toppingPrices.get(id),
        );
        if (
          !variant ||
  ...
  const shiftQuery = useQuery({
    ...
    queryFn: () =>
      api.get("/shifts/current").then((response) => response.data.data),
  });
  const draftsQuery = useQuery({
    queryKey: ["my-drafts"],
    queryFn: () =>
      api.get("/orders/drafts/mine").then((response) => response.data.data),
    enabled: draftsOpen,
  });
  ...
  const localTotal = cart.reduce(
    (sum, line) => sum + line.displayUnitPrice * line.quantity,
    0,
  );
    ...
    <div className="tw-flex tw-min-h-full tw-flex-col tw-bg-[#f3f6f5] xl:tw-h-full xl:tw-flex-row dark:tw-bg-[#0b1211]">
      <section className="soft-scrollbar tw-min-w-0 tw-flex-1 tw-p-4 sm:tw-p-5 xl:tw-overflow-y-auto">
        <div className="tw-sticky tw-top-0 tw-z-20 tw-mb-5 tw-space-y-3 tw-rounded-2xl tw-border tw-border-slate-200 tw-bg-white/95 tw-p-3.5 tw-shadow-panel tw-backdrop-blur-xl dark:tw-border-slate-700 dark:tw-bg-slate-900/95">
          <div className="tw-flex tw-flex-col tw-gap-3 sm:tw-flex-row">
            ...
            />
            <div
              className={`tw-flex tw-shrink-0 tw-items-center tw-gap-2 tw-rounded-xl tw-border tw-px-3.5 tw-py-2 tw-text-xs tw-font-bold ${
                shiftQuery.data
                  ? "tw-border-emerald-200 tw-bg-emerald-50 tw-text-emerald-700 dark:tw-border-emerald-800 dark:tw-bg-emerald-900/20"
                  : "tw-border-rose-200 tw-bg-rose-50 tw-text-rose-600 dark:tw-border-rose-800 dark:tw-bg-rose-900/20"
              }`}
            >
              <Clock3 size={17} />
              {shiftQuery.data ? (
                `Ca ${shiftQuery.data.code}`
              ) : (
                <Link to="/shifts" className="tw-text-inherit">
                  Mở ca để bán hàng
                </Link>
              )}
            </div>
          ...
          <div className="no-scrollbar tw-flex tw-gap-2 tw-overflow-x-auto">
            <button
              ...
              className={`tw-shrink-0 tw-rounded-lg tw-border tw-px-3.5 tw-py-2 tw-text-xs tw-font-bold tw-transition ${
                categoryId === ""
                  ? "tw-border-mint-700 tw-bg-mint-700 tw-text-white tw-shadow-sm"
                  : "tw-border-slate-200 tw-bg-white tw-text-slate-500 hover:tw-border-mint-300 hover:tw-text-mint-700 dark:tw-border-slate-700 dark:tw-bg-slate-800 dark:tw-text-slate-300"
              }`}
            >
              Tất cả
            </button>
        ...
        {productsQuery.isLoading ? (
          ...
          <EmptyState
            title="Không tìm thấy món kem"
            description="Thử từ khóa hoặc danh mục khác."
          />
        )}
      </section>
      ...
      <aside className="tw-flex tw-w-full tw-flex-col tw-border-t tw-border-slate-200 tw-bg-white tw-shadow-[-10px_0_30px_rgba(15,52,46,0.04)] xl:tw-h-full xl:tw-w-[420px] xl:tw-border-l xl:tw-border-t-0 dark:tw-border-slate-800 dark:tw-bg-slate-900">
        <div className="tw-flex tw-items-center tw-justify-between tw-border-b tw-border-slate-100 tw-px-5 tw-py-4 dark:tw-border-slate-800">
          <div className="tw-flex tw-items-center tw-gap-3">
            <div className="tw-flex tw-h-10 tw-w-10 tw-items-center tw-justify-center tw-rounded-xl tw-bg-[#0b2924] tw-text-white dark:tw-bg-mint-700">
              <ShoppingCart size={20} />
            </div>
            <div>
              <strong className="tw-block tw-text-[15px] tw-font-extrabold">
                Đơn hiện tại
              </strong>
              <span className="tw-text-[11px] tw-text-slate-400">
                {cart.reduce((sum, line) => sum + line.quantity, 0)} món đã chọn
              </span>
            </div>
          </div>
          <Button
        ...
        <div className="soft-scrollbar tw-min-h-64 tw-flex-1 tw-overflow-y-auto tw-bg-[#fbfcfc] tw-p-4 dark:tw-bg-slate-950/25">
          {!cart.length ? (
            <EmptyState
              ...
              description="Chạm vào món kem bên trái để bắt đầu."
            />
          ) : (
            <div className="tw-space-y-3">
              {cart.map((line) => (
                <div
                  key={line.cartId}
                  className="tw-rounded-xl tw-border tw-border-slate-200 tw-bg-white tw-p-3 tw-shadow-[0_1px_2px_rgba(15,23,42,0.025)] dark:tw-border-slate-700 dark:tw-bg-slate-900"
                >
                  <div className="tw-flex tw-gap-3">
                    <div className="tw-flex tw-h-12 tw-w-12 tw-shrink-0 tw-items-center tw-justify-center tw-overflow-hidden tw-rounded-lg tw-bg-mint-50 dark:tw-bg-slate-800">
                      {line.imageUrl ? (
                        ...
                        <IceCreamBowl size={22} className="tw-text-mint-500" />
                      )}
                    </div>
                    <div className="tw-min-w-0 tw-flex-1">
                      <div className="tw-flex tw-items-start tw-justify-between tw-gap-2">
                        <div>
                          <strong className="tw-block tw-truncate tw-text-sm">
                            {line.productName}
                          </strong>
                      ...
                      <div className="tw-mt-2 tw-flex tw-items-center tw-justify-between">
                        <div className="tw-flex tw-items-center tw-gap-1 tw-rounded-lg tw-border tw-border-slate-200 tw-bg-slate-50 tw-p-0.5 dark:tw-border-slate-700 dark:tw-bg-slate-800">
                          ...
                          <span className="tw-w-6 tw-text-center tw-text-sm tw-font-black">
                            {line.quantity}
                          </span>
                          <IconButton
                            size="small"
                        ...
                        <strong className="tw-text-sm tw-text-mint-700">
                          {formatMoney(line.displayUnitPrice * line.quantity)}
                        </strong>
                      </div>
                    </div>
                  </div>
                  {line.note && (
                    <div className="tw-mt-2 tw-flex tw-items-center tw-gap-1 tw-text-xs tw-text-slate-400">
                      <NotebookPen size={13} /> {line.note}
                    </div>
                  )}
                </div>
              ))}
        ...
        <div className="tw-border-t tw-border-slate-200 tw-bg-white tw-p-4 dark:tw-border-slate-800 dark:tw-bg-slate-900">
          ...
          >
            ...
            >
              <span className="tw-text-sm tw-font-black">
                Khách hàng & ưu đãi
              </span>
            </AccordionSummary>
            <AccordionDetails sx={{ px: 0, pt: 1 }}>
              <div className="tw-space-y-3">
                ...
                {branchVouchers.length > 0 && (
                  <div>
                    <div className="tw-mb-2 tw-flex tw-items-center tw-gap-1.5 tw-text-[11px] tw-font-bold tw-uppercase tw-tracking-wide tw-text-slate-500">
                      <Gift size={14} className="tw-text-mint-600" /> Voucher
                      dùng tại {user?.branch?.name}
                    </div>
                ...
                {promotionsQuery.data?.some(
                  (promotion) => promotion.type === "BUY_X_GET_Y",
                ) && (
                  <div>
                    <div className="tw-mb-2 tw-flex tw-items-center tw-gap-1.5 tw-text-[11px] tw-font-bold tw-uppercase tw-tracking-wide tw-text-slate-500">
                      <Sparkles size={14} className="tw-text-amber-500" /> Ưu
                      đãi nhanh
                    </div>
                    <div className="tw-space-y-2">
                            ...
                            >
                              ...
                              <span className="tw-min-w-0 tw-flex-1">
                                ...
                                <span className="tw-block tw-truncate tw-text-[11px] tw-text-slate-500">
                                  Mua {promotion.buyQuantity} tặng{" "}
                                  {promotion.getQuantity} · Mã {promotion.code}
                                </span>
                              </span>
                              <span
                                className={`tw-rounded-full tw-px-2.5 tw-py-1 tw-text-[10px] tw-font-black ${
                                  selected
                ...
                <Input
                  ...
                  value={deliveryFee}
                  onChange={(event) =>
                    setDeliveryFee(Math.max(0, Number(event.target.value)))
                  }
                />
                <Input
                  label="Ghi chú toàn đơn"
                  multiline
                  rows={2}
                  value={orderNote}
          ...
          {quote?.voucher && (
            <div className="tw-mb-3 tw-flex tw-items-start tw-gap-2.5 tw-rounded-xl tw-border tw-border-mint-200 tw-bg-mint-50 tw-p-3 dark:tw-border-mint-800 dark:tw-bg-mint-900/20">
              <Gift
                size={18}
                className="tw-mt-0.5 tw-shrink-0 tw-text-mint-600"
              />
              <div className="tw-min-w-0 tw-flex-1">
                ...
                <span className="tw-mt-0.5 tw-block tw-text-[11px] tw-text-mint-700/80 dark:tw-text-mint-300/80">
                  {quote.voucher.branch.name} · Giảm{" "}
                  {formatMoney(quote.voucherDiscount)}
                </span>
              </div>
              <button
                type="button"
          ...
          {quote?.activeMembership && quote?.membershipBenefit && (
            <div className="tw-mb-3 tw-flex tw-items-start tw-gap-2.5 tw-rounded-xl tw-border tw-border-lavender-200 tw-bg-lavender-50 tw-p-3 dark:tw-border-lavender-800 dark:tw-bg-lavender-900/20">
              <Crown
                size={18}
                className="tw-mt-0.5 tw-shrink-0 tw-text-lavender-500"
              />
              <div className="tw-min-w-0 tw-flex-1">
                ...
                {!quote.membershipBenefit.usedToday &&
                  !quote.membershipBenefit.available &&
                  quote.activeMembership.plan.benefitVariant && (
                    <Button
                      size="small"
                      variant="text"
          ...
          <div className="tw-space-y-2 tw-rounded-xl tw-bg-slate-50 tw-p-3 tw-text-xs dark:tw-bg-slate-800/70">
            <div className="tw-flex tw-justify-between">
              <span className="tw-text-slate-500">Tiền hàng</span>
              <span>{formatMoney(quote?.originalAmount ?? localTotal)}</span>
            </div>
            <div className="tw-flex tw-justify-between">
              <span className="tw-text-slate-500">Giảm giá</span>
              <span>
                -
                {formatMoney(
            ...
            <div className="tw-flex tw-justify-between">
              ...
              <span>{formatMoney(quote?.taxAmount || 0)}</span>
            </div>
            <div className="tw-flex tw-justify-between">
              <span className="tw-text-slate-500">Phí giao hàng</span>
              <span>{formatMoney(quote?.deliveryFee || deliveryFee)}</span>
            </div>
            <div className="tw-flex tw-items-end tw-justify-between tw-border-t tw-border-dashed tw-border-slate-300 tw-pt-3 dark:tw-border-slate-700">
              <strong className="tw-text-[13px]">Tổng thanh toán</strong>
              <strong className="tw-text-[25px] tw-font-extrabold tw-tracking-[-0.04em] tw-text-mint-700 dark:tw-text-mint-300">
                {formatMoney(total)}
              </strong>
            </div>
          </div>
                  ...
                  <button
                    ...
                    className={`tw-flex tw-flex-col tw-items-center tw-gap-1 tw-rounded-xl tw-border tw-p-2.5 tw-text-[10px] tw-font-bold tw-transition ${
                      paymentMethod === method
                        ? "tw-border-mint-600 tw-bg-mint-50 tw-text-mint-700 tw-shadow-sm dark:tw-bg-mint-700/20"
                        : "tw-border-slate-200 tw-bg-transparent tw-text-slate-500 hover:tw-border-slate-300 dark:tw-border-slate-700"
                    }`}
                  >
                    <Icon size={18} />
          ...
          {paymentMethod === "CASH" && cart.length > 0 && (
            <div className="tw-mb-3 tw-grid tw-grid-cols-2 tw-gap-2">
              <Input
                ...
                value={customerPaid}
                onChange={(event) =>
                  setCustomerPaid(Math.max(0, Number(event.target.value)))
                }
              />
          ...
          <div className="tw-grid tw-grid-cols-[1fr_1fr_2fr] tw-gap-2">
            ...
            <Button
              variant="outlined"
              disabled={!cart.length || !shiftQuery.data}
              onClick={() => saveOrderMutation.mutate(true)}
              aria-label="Lưu tạm"
            >
              <Save size={18} />
            </Button>
            <Button
              ...
              loading={
                ...
                saveOrderMutation.variables === false
              }
              onClick={checkout}
            >
              Thanh toán
          ...
          <div className="tw-space-y-3">
            {draftsQuery.data.map((draft) => (
              <button
                ...
                className="tw-flex tw-w-full tw-items-center tw-gap-3 tw-rounded-2xl tw-border tw-border-slate-100 tw-bg-transparent tw-p-4 tw-text-left hover:tw-bg-mint-50 dark:tw-border-slate-700 dark:hover:tw-bg-mint-700/10"
              >
                <Receipt size={20} className="tw-text-mint-600" />
                <div className="tw-flex-1">
                  <strong className="tw-block">{draft.code}</strong>
                  <span className="tw-text-xs tw-text-slate-400">
                    {formatDate(draft.updatedAt, true)} · {draft.items.length}{" "}
                    dòng món
                  </span>
                </div>
          ...
          <>
            <Button
              ...
              onClick={() =>
                completedOrder &&
                downloadFile(
                  `/orders/${completedOrder.id}/invoice.pdf`,
                  `${completedOrder.code}.pdf`,
                )
              }
            >
              Tải hóa đơn PDF
      ...
      >
        <div className="tw-flex tw-flex-col tw-items-center tw-py-5 tw-text-center">
          ...
          <h3 className="tw-mb-1 tw-mt-4 tw-text-2xl tw-font-black">
            {completedOrder?.code}
          </h3>
          <p className="tw-m-0 tw-text-slate-500">
            Đơn hàng đã lưu, kho và điểm xếp hạng đã được cập nhật.
          </p>
          <strong className="tw-mt-5 tw-text-3xl tw-text-mint-700">
            {formatMoney(completedOrder?.totalAmount)}
          </strong>
          {completedOrder?.issuedVouchers?.map((voucher) => (
            <div
              key={voucher.id}
              className="tw-mt-5 tw-w-full tw-rounded-2xl tw-border tw-border-amber-200 tw-bg-amber-50 tw-p-4 tw-text-left dark:tw-border-amber-800 dark:tw-bg-amber-900/20"
            >
              <div className="tw-flex tw-items-center tw-gap-2 tw-font-black tw-text-amber-800 dark:tw-text-amber-200">
                <Gift size={18} /> Khách vừa nhận voucher hạng{" "}
                {voucher.membershipLevel.name}
              </div>
              <div className="tw-mt-2 tw-flex tw-items-end tw-justify-between tw-gap-3">
                <strong className="tw-text-lg tw-tracking-wide">
                  {voucher.code}
                </strong>
