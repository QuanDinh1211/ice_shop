...
export default function ShiftsPage() {
  ...
  const openMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Đã mở ca, bạn có thể tạo đơn");
      refresh();
    },
  ...
  const closeMutation = useMutation({
    mutationFn: () =>
      api.post(`/shifts/${current.id}/close`, {
  ...
  const expenseMutation = useMutation({
    mutationFn: () =>
      api.post(`/shifts/${current.id}/expenses`, {
        ...expense,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value) => (
        <strong
          className={value > 0 ? "tw-text-rose-500" : "tw-text-slate-400"}
        >
          {formatMoney(value)}
        </strong>
      ),
    },
    {
      ...
      render: (value) => (
        <StatusBadge
          status={value}
          label={value === "OPEN" ? "Đang mở" : "Đã đóng"}
        />
      ),
    },
    {
  ...
  return (
    <div className="tw-space-y-6 tw-p-4 sm:tw-p-6 xl:tw-p-8">
      ...
      {current ? (
        <section className="professional-grid tw-rounded-2xl tw-bg-[#0b3a32] tw-p-6 tw-text-white tw-shadow-floating">
          <div className="tw-flex tw-flex-col tw-gap-5 sm:tw-flex-row sm:tw-items-start sm:tw-justify-between">
            <div>
              <span className="tw-inline-flex tw-items-center tw-gap-2 tw-rounded-lg tw-border tw-border-white/10 tw-bg-white/[0.07] tw-px-3 tw-py-1.5 tw-text-[10px] tw-font-bold tw-uppercase tw-tracking-wider">
                <span className="tw-h-1.5 tw-w-1.5 tw-rounded-full tw-bg-emerald-300" />{" "}
                CA ĐANG MỞ
              </span>
              <h2 className="tw-mb-1 tw-mt-4 tw-text-2xl tw-font-black">
                {current.code}
              </h2>
              <p className="tw-m-0 tw-text-sm tw-text-white/75">
                {current.branch.name} · mở lúc{" "}
                {formatDate(current.openedAt, true)}
              </p>
            </div>
          ...
          <div className="tw-mt-6 tw-grid tw-gap-3 sm:tw-grid-cols-2 lg:tw-grid-cols-4">
            {[
              ...
              <div
                key={label}
                className="tw-rounded-xl tw-border tw-border-white/10 tw-bg-white/[0.07] tw-p-4 tw-backdrop-blur"
              >
                <Icon size={20} className="tw-mb-3" />
                <span className="tw-block tw-text-xs tw-text-white/70">
                  {label}
                </span>
                <strong className="tw-mt-1 tw-block tw-text-xl">
                  {count ? value : formatMoney(value)}
                </strong>
              </div>
            ))}
          </div>
          <div className="tw-mt-4 tw-grid tw-gap-2 tw-text-sm sm:tw-grid-cols-3">
            <div>
              Hoàn trả: <strong>{formatMoney(current.refundAmount)}</strong>
            </div>
            <div>
              Chi phí: <strong>{formatMoney(current.expenseAmount)}</strong>
            </div>
            <div>
            ...
            action={
              <div className="tw-mt-2 tw-w-full tw-max-w-sm tw-space-y-3">
                <Input
                  ...
                  value={openingCash}
                  onChange={(event) =>
                    setOpeningCash(Math.max(0, Number(event.target.value)))
                  }
                />
                <Input
                  label="Ghi chú"
      ...
      <section className="tw-rounded-2xl tw-border tw-border-slate-200 tw-bg-white tw-p-4 tw-shadow-[0_1px_2px_rgba(15,23,42,0.025)] dark:tw-border-slate-700 dark:tw-bg-slate-900">
        <h3 className="tw-mb-4 tw-mt-1 tw-text-lg tw-font-black">
          Lịch sử ca làm việc
        </h3>
        <DataTable
          columns={columns}
        ...
        actions={
          <>
            ...
            <Button
              ...
              disabled={expense.amount <= 0 || !expense.description.trim()}
        ...
        <div className="tw-space-y-4 tw-pt-2">
          <div className="tw-grid tw-gap-3 sm:tw-grid-cols-3">
            <div className="tw-rounded-xl tw-bg-slate-50 tw-p-3 dark:tw-bg-slate-800">
              ...
              <strong>{historyShift?.user?.fullName}</strong>
            </div>
            <div className="tw-rounded-xl tw-bg-slate-50 tw-p-3 dark:tw-bg-slate-800">
              <span className="tw-block tw-text-xs tw-text-slate-500">
                Chi nhánh
              </span>
              <strong>{historyShift?.branch?.name}</strong>
            </div>
            <div className="tw-rounded-xl tw-bg-rose-50 tw-p-3 dark:tw-bg-rose-950/30">
              <span className="tw-block tw-text-xs tw-text-slate-500">
                Tổng chi phí
              </span>
              <strong className="tw-text-rose-600">
                {formatMoney(historyShift?.expenseAmount || 0)}
              </strong>
            </div>
          </div>
          {(historyShift?.expenses || []).length ? (
            <div className="tw-overflow-hidden tw-rounded-xl tw-border tw-border-slate-200 dark:tw-border-slate-700">
              {historyShift.expenses.map((item) => (
                <div
                  key={item.id}
                  className="tw-flex tw-flex-col tw-gap-2 tw-border-b tw-border-slate-100 tw-p-4 last:tw-border-b-0 sm:tw-flex-row sm:tw-items-center dark:tw-border-slate-800"
                >
                  <div className="tw-min-w-0 tw-flex-1">
                    <strong className="tw-block tw-break-words">
                      {item.description}
                    </strong>
                  ...
                  <strong className="tw-whitespace-nowrap tw-text-rose-600">
            ...
            <EmptyState
              ...
              description="Ca làm việc này chưa ghi nhận khoản chi nào."
            />
          )}
        </div>
      </Modal>
      ...
      >
        <div className="tw-space-y-4 tw-pt-2">
          ...
          <Input
            ...
            value={countedCash}
            onChange={(event) =>
              setCountedCash(Math.max(0, Number(event.target.value)))
            }
          />
          <div className="tw-text-sm">
            ...
            <strong
              className={
                countedCash - expectedCash === 0
                  ? "tw-text-emerald-600"
                  : "tw-text-rose-500"
              }
            >
              {formatMoney(countedCash - expectedCash)}
            </strong>
          </div>
        </div>
      </Modal>
